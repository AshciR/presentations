...
public class TryWithResources {
    ...
    must be closed by calling a .close() method.
    ...
    void manuallyCloseResources() {
        ...
        try {
            ...
            e.printStackTrace();
        } finally { // People could handle the closing in a finally block
            if (scanner != null) {
    ...
    /*
    It gets the job done, but there's boilerplate code.
    If we can avoid writing boilerplate code that distracts from
    ...
    void manuallyCloseMultipleResources() {
        ...
        try { // try for the input scanner
            scanner = new Scanner(new File(pathToSrcFile));

            try { // try for the writer
    ...
    /*
     We would have nested try blocks and
     it starts to look like JS callback hell.
     We're not front-end devs 🙃
     */
    ...
    /*
    Notice the resource that was defined first is closed last.
